...
parser.add_argument('--checkpoint-path', type=str, default='checkpoints/train_sst2_on_mixtext_1_percent_no_augmentations/model_best.pth', help='Saved model checkpoint')
...
def optimization_function(input_arguments):
    ...
    val_dataset = create_dataset(val['X'], val['y'], model_name, 256, mix='Inter_LADA', num_classes=2,knn_lada=knn, mu_lada=mu, dataset_identifier=dataset_identifier)
...
if __name__ == "__main__":
    ...
    pickle.dump(
        trials,
        open(f'data/saved_logs/sst2_hyperopt_single_inter_lada_layers_changes_{args.number_of_policies_to_evaluate}.pkl', 'wb'))
